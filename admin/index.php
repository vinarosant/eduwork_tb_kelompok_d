...
(id,judul,tgl_publish,isi,gambar,id_kategori,id_penulis,id_admin) 
VALUES (NULL,'$judul','$tgl_publish','$isi','$xx','$id_kategori','$id_penulis','$id_admin')");
...
if(isset($_POST['EditBerita'])){
  ...
  if($foto_baru != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
        if(is_file("berita/".$foto_baru)){
            unlink("berita/".$foto_baru);
        }
                  move_uploaded_file($file_tmp, 'berita/'.$nama_gambar_baru); 
                    
                   $query  = "UPDATE berita SET 
                   id = '$id', judul = '$judul', 
                   tgl_publish='$tgl_publish', isi = '$isi', gambar = '$nama_gambar_baru', id_kategori = '$id_kategori', 
                   id_penulis = '$id_penulis',id_admin = '$id_admin'";
                    $query .= "WHERE id = '$id'";
      ...
      $query  = "UPDATE berita SET  id = '$id', judul = '$judul', 
           ...
           <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                      ...
                      LEFT JOIN kategori ON `kategori`.`id_kategori`=`berita`.`id`
                      LEFT JOIN penulis ON `penulis`.`id_penulis`=`berita`.`id`
                      LEFT JOIN admin ON `admin`.`id_admin`=`berita`.`id`");
                      $no=1;
                  ...
                  <tr>
                    <td><?= $no++; ?></td>
                    <td>
                      <img src="images/<?= $d['gambar']; ?>" width="120" height="120">
                    </td>
                    <td><?= $d['judul']; ?></td>
                    <td><?= $d['tgl_publish']; ?></td>
                    <td><?= $d['isi']; ?></td>
              ...
              ?>
                <div class="card-body">
                  <div class="form-group">
                    <label for="ID Berita">No</label>
                    <input type="text" class="form-control" id="id_obat" value="<?= $row['id']; ?>" name="id" readonly>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <input type="datetime" class="form-control" id="tanggal" value="<?= $row['tgl_publish']; ?>" name="tanggal" required>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    ?>
                  <option value="<?php echo $kategori['id_kategori']; ?>" <?php if($row['id_kategori']==$kategori['id_kategori']){ echo "selected"; } ?> ><?php echo $kategori['kategori']; ?></option>
                  <?php } ?>
                  </select>
                  </div>
                  <div class="form-group">
                    ...
                    ?>
                  <option value="<?php echo $penulis['id_penulis']; ?>" <?php if($row['id_penulis']==$penulis['id_penulis']){ echo "selected"; } ?> ><?php echo $penulis['nama']; ?></option>
                  <?php } ?>
                  </select>
                  </div>
                  <div class="form-group">
                    ...
                    <?php
                    $adm = mysqli_query($koneksi,"SELECT * FROM `admin`");
                    while($admin=mysqli_fetch_assoc($adm)){
                    ?>
                  <option value="<?php echo $admin['id_admin']; ?>" <?php if($row['id_admin']==$admin['id_admin']){ echo "selected"; } ?> ><?php echo $admin['username']; ?></option>
                  <?php } ?>
                  </select>
                  </div>
